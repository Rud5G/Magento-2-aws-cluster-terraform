...
if [ ${INSTANCE_NAME} != "staging" ]; then
  sudo git clone -b main ${CODECOMMIT_APP_REPO} ${WEB_ROOT_PATH}/
  ...
  sudo git clone -b staging ${CODECOMMIT_APP_REPO} ${WEB_ROOT_PATH}/
...
sudo sed -i 's/^\(memory_limit = \)[0-9]*M/\1%{ if "${INSTANCE_NAME}" == "admin" }2048M%{ else }756M%{ endif }/' ${PHP_INI}
...
sudo sed -i "s/INSTANCE_LOCAL_IP/${INSTANCE_LOCAL_IP}/" /etc/nginx/sites-available/magento.conf
